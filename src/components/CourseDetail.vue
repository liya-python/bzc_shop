            ...
            <div class="course-info">
                ...
                <div class="wrap-right">
                    ...
                    <div class="sale-time" v-if="!course.discount_name==''">
                        ...
                        <p class="expire">距离结束：仅剩 {{dd}}天 {{hh}}小时 {{mm}}分 <span class="second">{{ss}}</span> 秒</p>
                    </div>
...
<!--                        <span class="original">¥{{course.price}}</span>-->
                    ...
                    <div class="buy">
                        ...
                        <div class="add-cart"><img src="/static/image/cart.svg" alt="" @click="add_cart(course.id)">加入购物车</div>
                    </div>
                </div>
            </div>
            ...
            <div class="course-content">
                <div class="course-tab-list">
                    ...
                    <div class="tab-item" v-if="tabIndex==2">
                        ...
                        <div class="chapter-item" v-for="(c_chapter,index) in chapter_list" :key="index">
                            <p class="chapter-title" @click="click_title"><img src="/static/image/12.png" alt="">第{{c_chapter.chapter}}章·{{c_chapter.name}}</p>
                            <ul class="lesson-list" v-show="show">
                                <li class="lesson-item" v-for="(lesson) in c_chapter.coursesections">
                                    <p class="name" ><span class="index">{{c_chapter.chapter}}-{{lesson.orders}}</span>{{lesson.name}}<span class="free">{{lesson.free_trail==1?'免费':'付费'}}</span>
                                    </p>
                                    <p class="time">07:30 <img src="/static/image/chapter-player.svg"></p>
                                    <button class="try">{{lesson.free_trail==1?'立即试学':'付费可学'}}</button>
                                </li>
                            </ul>
                        </div>
                    </div>
    ...
    export default {
        ...
        methods:{
            ...
            add_cart(course_id) {
                ...
                // 发起请求添加商品
                this.$axios.post("http://127.0.0.1:9000/cart/option/", {
                    course_id: course_id
                },{
                    headers:{
            ...
            get_course_chapter(){
                this.$axios({
                    url:'http://127.0.0.1:9000/course/chapter/?course='+this.course_id,
                    method: 'get',
                }).then(response =>{
                    // console.log(response.data[0].coursesections);
            ...
            get_course_detail(){
                this.$axios({
                    url: 'http://127.0.0.1:9000/course/detail/'+this.course_id+'/',
                    method: 'get',
                }).then(response =>{
                    // console.log(response.data.discount_end_time);
                    this.course=response.data;// 每个课程要播放的视频
                    this.end_time = response.data.discount_end_time
                    this.get_time(this.end_time);
